...
class Elliptic:
    ...
    def get_n(self):
        ...
        while self.n:
            self.n += 1
            temp_x, temp_y = Elliptic.step(
                self, temp_x, temp_y, self.xGround, self.yGround
            )
    ...
    def encrypt2(self, plain):
        ...
        for char in plain:
            ...
            kQx, kQy = Elliptic.get_ng(
                self, self.public_key[0], self.public_key[1], r
            )  # kQ
            cipher = intchar * kQx
            output.append([kGx, kGy, cipher])
        f3 = open("Assets/Output/encrypt.txt", "w")
